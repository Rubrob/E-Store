...
module.exports = {
  ...
  getProducts: async (req, res) => {
    try {
      const { color, size, page, sort = "price_desc" } = req.query;
      const [sortType, sortValue] = sort.split("_");
      // const filterColors = () => {
      //   const obj = {};
      //   if (color) obj["color"] = color;
      //   if (size) obj["sizes.size"] = size;
      //   return obj;
      // };

      // const productFilters = () => {
      //   const obj = {};
      //   if (req.params.category_slug) obj["category_slugs"] = req.params.category_slug;
      //   if (req.query.search) obj["title"] = new RegExp("^" + req.query.search, "i");
      //   return obj;
      // };
      ...
      await models.Product.find(
        filterEmpty({
          ...
          title: req.query.search ? new RegExp("^" + req.query.search, "i") : undefined
        })
      )
    ...
    models.Color.aggregate()
      ...
      .then(docs => {
        ...
        const result = docs.map(item => {
          const idx = skus.findIndex(skuItem => skuItem === item.sku);
          if (idx <= -1) {
            return {};
          }
          return {
